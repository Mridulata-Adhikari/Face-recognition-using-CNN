...
class Convolution:
    ...
    def convolve(self, input, filters, conv_shape):
        ...
        convolved_features = np.zeros(conv_shape)  # total number of convolved features

        for input_num in range(n_input):
            for channel_out in range(n_channel_out): # for each filter
                """ Image to be convolved """
                convolved_image = np.zeros((conv_height, conv_width))  # create an zero initialized convolved image
                for channel_in in range(n_channel_in): # for each feature map
                    ...
                    for y in range(image.shape[1]):
                        if y > image.shape[1]- filter_w:
                            break
                        # Only Convolve if y has gone down by the specified Strides
                        if y % 1 == 0:
                            for x in range(image.shape[0]):
                                # Go to next row once kernel is out of bounds
                                if x > image.shape[0] - filter_h:
                                    break
                                try:
                                    # Only Convolve if x has moved by the specified Strides
                                    if x % 1 == 0:
                                        convolved_image[x, y] = np.sum(filters * image[x: x + filter_h, y: y + filter_w])
                                except:
                                    break
    ...
    def convolve_backprop(self, input, convout_grad, filters, filters_grad):
        ...
        imgs_grad = np.zeros((n_imgs, n_channels_imgs, img_h, img_w))  # total number of convolved features
        filters_grad[...] = 0
                        ...
                        for y_off in range(y_off_min, y_off_max):
                            for x_off in range(x_off_min, x_off_max):
                                ...
                                for c_imgs in range(n_channels_imgs):
                                    imgs_grad[i, c_imgs, img_y, img_x] += filters[
                                                                              c_imgs, c_convout, fil_y, fil_x] * convout_grad_value
                                    filters_grad[c_imgs, c_convout, fil_y, fil_x] += (
                                                input[i, c_imgs, img_y, img_x] * convout_grad_value)
