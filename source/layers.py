import numpy as np
from source.convolution import Convolution
from source.pooling import Pooling
...
class Conv(Layer):
    ...
    def setup(self, input_shape, rng):
        ...
        self.b = np.zeros(self.n_filters) # initialize bias weights equal to zero
...
class Pool(Layer):
    ...
    def forward_propogation(self, input):
        ...
        self.last_maxpositions = np.empty(self.output_shape(input.shape)+(2,), dtype=np.int)  # positions which hold the maximum elements
...
class Activation(Layer):
    ...
    # def relu(self, x):
    #     z = np.log(1+np.exp(x))#np.maximum(0.0, x)
    #     return z
    #
    # def relu_d(self, x):
    #     dx = 1/(1+np.exp(-x))#np.zeros(x.shape)
    #     #dx[x >= 0] = 1
    #     return dx
...
class LogRegression(Layer):
    ...
    def valloss(self, Y_val, proba):
        # Assumes one-hot encoding.
        eps = 1e-15
        proba = np.clip(proba, eps, 1 - eps)
        proba /= proba.sum(axis=1, keepdims=True)
        loss = -np.sum(Y_val * np.log(proba))
